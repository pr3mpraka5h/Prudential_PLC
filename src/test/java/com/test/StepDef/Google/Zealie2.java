	...
	public static void main(String a[]) throws InterruptedException
	{
		...
		for(int i=0;i<l.size();i++)
		{
			...
			if(l.get(i).getText().toString().contains("Stephen Shaw"))
			{
				...
				assertEquals(row.get(2).getText().toString(),"Colombia1","Country Mismatch");
				assertEquals(row.get(3).getText().toString(),"Stephen Shaw1","Name Mismatch");
				assertEquals(row.get(5).getText().toString(),"$96,709.001","Balance Mismatch");
				assertEquals(row.get(6).getText().toString(),"PROPOSAL1","Activity Mismatch");
				break;
			}
//			if(l.get(i).getText().toString().compareTo("Ends with")==0)
//				l.get(i).click();
